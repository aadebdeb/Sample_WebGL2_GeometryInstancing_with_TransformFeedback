...
void main(void) {
  ...
  o_color = vec4(v_color * max(0.0, smoothstep(-1.0, 1.0, dotNL)), 1.0);
}
`
  ...
  const renderUniforms = getUniformLocations(gl, renderProgram, ['u_mvpMatrix']);

  let sphere, sphereNum;
  let positions, velocities, positionVbo, velocityVbo, vao;
  const reset = function() {
    ...
    positionVbo = createVbo(gl, positions, gl.DYNAMIC_DRAW);
    velocityVbo = createVbo(gl, velocities, gl.DYNAMIC_DRAW);
    const colorVbo = createVbo(gl, colors);

    vao = gl.createVertexArray();
    gl.bindVertexArray(vao);
    gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, sphereIbo);
    [vertexPositionVbo, vertexNormalVbo, colorVbo, positionVbo].forEach((vbo, i) => {
      gl.bindBuffer(gl.ARRAY_BUFFER, vbo);
  ...
  const gravity = new Vector3(0.0, -9.8, 0.0);
  const mass = 10.0;
  ...
  const render = function() {
    ...
    // update particles
    for (let i = 0; i < sphereNum; i++) {
        position = new Vector3(positions[3 * i], positions[3 * i + 1], positions[3 * i + 2]);
        velocity = new Vector3(velocities[3 * i], velocities[3 * i + 1], velocities[3 * i + 2]);
        velocity.add(Vector3.mul(gravity, elapsedTime / mass));
        position.add(Vector3.mul(velocity, elapsedTime));
